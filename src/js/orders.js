...
/**
 * Adds a new order to the database, optionally linking it to a borrower.
 * This process is wrapped in a transaction to ensure data integrity. It creates an order,
 * creates snapshots of the products at the time of purchase, updates the stock levels,
 * and links the order to a borrower if specified.
 ...
 * @param {number|null} borrowerId - The ID of the borrower to link to the order, or null if none.
 * @returns {Promise<number>} The ID of the newly created order.
 */
export async function addOrder(productsList, borrowerId = null) {
  const db = getDb();
  ...
  try {
    ...
    // 5. Link to borrower if specified
    if (borrowerId !== null) {
      const borrowerCheckStmt = db.prepare("SELECT id FROM borrowers WHERE id = ?;");
      const borrowerExists = borrowerCheckStmt.get([borrowerId]);
      borrowerCheckStmt.free();
      if (!borrowerExists) {
        throw new Error(`Borrower with ID ${borrowerId} not found.`);
      }
      const snapshotInsertStmt = db.prepare("INSERT INTO orders_snapshots (borrower_id, original_order_id, created_at) VALUES (?, ?, ?);");
      snapshotInsertStmt.run([borrowerId, orderId, now]);
    ...
    db.exec("COMMIT;");
    await saveDb();
    console.log(`✅ Order #${orderId} created successfully${borrowerId ? ` and linked to borrower #${borrowerId}` : ''}.`);
    return orderId;
