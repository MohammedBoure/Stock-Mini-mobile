...
export async function getSalesDataForChart(duration = '7', startDate = null, endDate = null, accuracy = 'auto') {
  ...
  if (duration === 'custom' && startDate && endDate) {
    ...
    if (!vals[0] || !vals[1]) {
      ...
      return { labels: [today], salesData: [0], profitData: [0], accuracy: 'daily', totalSales: 0, totalProfit: 0 };
    }
    start = vals[0];
    end = vals[1];
  } else {
  ...
  if (accuracy === 'daily') {
    ...
    perDay.forEach(({ day, sales, profit }) => {
      ...
      if (!buckets.has(key)) buckets.set(key, { sales: 0, profit: 0 });
  ...
  return { labels, salesData, profitData, accuracy, totalSales, totalProfit };
}
